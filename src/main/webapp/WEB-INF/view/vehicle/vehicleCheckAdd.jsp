...
<script type="text/javascript">
	var $form;
	var $checkForm;
	var upUrl = getContextPath() + '/vehicle/saveCheck';
	var pkey;
	var opr;
	$(function() {
		$form = $("#form");
	...
	function newOrEidt() {
		...
		opr = getUrlParam("opr");
		if (pkey) {
			upUrl = getContextPath() + '/vehicle/editCheck';
			$.ajax({
				type : "post",
				dataType : "json",
				url : '../vehicle/queryOneCheck?id=' + pkey,
				async : true,
	...
	function loadVehicle(opretaCertNum) {
				...
				type : "post",
				dataType : "json",
				url : '../vehicle/queryOneByOpr?opretaCertNum=' + opretaCertNum,
				async : true,
				success : function(result) {
					if (result) {
						$form.form("load", result);
						//document.getElementById("uploadPreviewOwnerNamePic").src = "../vehicle/getOwnernamepic?id=" + pkey;
						$("#uploadPreviewOwnerNamePic").attr("src","../vehicle/getOwnernamepic?operatingnum=" + opr);
						$("#uploadPreviewVehiclePic").attr("src","../vehicle/getVehiclepic?operatingnum=" + opr);
					} else {
						showError("系统异常");
					}
				}
	...
	function back() {
		self.location.href = getContextPath() + "/vehicle/vehicleCheckListPage";
	}
	function save() {
		...
		$.ajax({
			...
			success : function(result) {
				if (result) {
					...
					//document.getElementById("uploadPreviewOwnerNamePic").src = "../vehicle/getOwnernamepic?id=" + pkey;
					$("#uploadPreviewOwnerNamePic").attr("src","../vehicle/getOwnernamepic?operatingnum=" + opretaCertNum);
					$("#uploadPreviewVehiclePic").attr("src","../vehicle/getVehiclepic?operatingnum=" + opretaCertNum);
				} else {
					showError("未找到相关信息");
				}
			}
